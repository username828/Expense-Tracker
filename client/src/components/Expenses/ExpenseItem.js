...
const ExpenseItem = ({ expense }) => {
    ...
    const handleDelete=async()=>{
        try{
            await axios.delete(`http://localhost:5000/api/delete-expense/${expense._id}`,
    ...
    return (
        ...
        <div className="flex flex-wrap items-center justify-between p-4 mb-4 bg-white shadow-md rounded-md border border-gray-200 hover:shadow-lg transition-shadow">
          ...
          <div className="flex gap-2 w-full sm:w-auto sm:justify-end">
            ...
            <button className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md transition-colors text-sm" onClick={handleDelete}>
              Delete
            </button>
          </div>
