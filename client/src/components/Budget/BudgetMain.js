...
const BudgetManager = () => {
  ...
  const fetchBudgets = async () => {
    try {
      const response = await axios.get(
        `${process.ENV.BASE_URL}/api/get-budgets`,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.ENV.BASE_URL}/api/delete-budget/${id}`, {
        withCredentials: true,
      });
        ...
        <div className="p-4 space-y-2 bg-white rounded-lg shadow-inner">
          ...
          {budgets.length > 0 ? (
            budgets.map((budget) => (
              <div
                key={budget._id}
                className="flex flex-wrap justify-between items-center p-2 bg-white rounded shadow-md"
              >
